...
import numpy as np
from boltons.iterutils import pairwise, windowed, chunked
...
def to_var(x, device):
    ...
    return to_cuda(x, device).requires_grad_()
...
def remove_overlapping(sorted_spans):
    ...
    nonoverlapping, seen = [], []
    for s in sorted_spans:
        indexes = [x for x in range(s.i1, s.i2+1)]
        ok = True
        for j in seen:
            cond1 = indexes[0] < j[0] <= indexes[-1] < j[-1]
            cond2 = j[0] < indexes[0] <= j[-1] < indexes[-1]
            # taken = [(i in j) for i in indexes]
            # if len(j)>1 and len(set(taken)) == 2:
            if cond1 or cond2:
                ok = False
                break
        if ok:
...
def extract_gold_corefs(document):
    ...
    # gold_corefs = flatten([[(gold[c],gold[c+1])
    #                         for c in range(len(gold)-1)]
