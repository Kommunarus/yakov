...
class Document:
    ...
    def sents(self):
        ...
        sent_idx = sent_idx + [len(self.tokens)]
    ...
    def spans(self):
        ...
        return [Span(i1=i[0], i2=i[-1], id=idx,
                ...
                for idx, i in enumerate(compute_idx_spans(self.sents, L=2))]
...
class LazyVectors:
    ...
    @classmethod
    def from_modelfastext(cls, corpus_vocabulary, name, cache):
...
def lookup_tensor(tokens, vectorizer, device):
    """ Convert a sentence to an embedding lookup tensor """
    return to_cuda(torch.tensor([vectorizer.stoi(t) for t in tokens]), device)

def rucorToCorpus(ruCor):
    return Corpus(load_cor(ruCor))
...
def load_cor(ruCor):
    ...
    speaker = 'speaker'
    ni = 0
    for i, doc_id in enumerate(ruCor.doc_ids):
        raw_text, utts_corefs, utts_speakers, filename = [], [], [], ''

        tokens = [x.wordform[0] for x in ruCor.texts[i]]
        sent_idx = [idx + 1
                    for idx, token in enumerate(tokens)
                    if token in ['.', '?', '!']]

        if len(sent_idx) < 50:
            utts_speakers.extend([speaker] * len(tokens))

            raw_text = ['Doc{}\t{}\t-'.format(doc_id, ni  ) for ii, token in enumerate(tokens)]

            label = 0
            for groups_index in ruCor.gs[i]['chains']:
                groups = ruCor.gs[i]['chains'][groups_index]
                for j in range(len(groups)):
                    dd = ruCor.gs[i]['groups'][groups[j]]['tokens_shifts']
                    s = ruCor.words_index[i][dd[0]]
                    e = ruCor.words_index[i][dd[-1]]
                    utts_corefs.append({'label': label, 'start': s, 'end': e, 'span': (s,e)})

                    startchars = raw_text[s][-1]
                    endchare = raw_text[e][-1]
                    if s == e:
                        if startchars == '-':
                            raw_text[s] = raw_text[s][:-1]+'({})'.format(label)
                        else:
                            raw_text[s] = raw_text[s]+'|({})'.format(label)

                    else:
                        for nn in range(s+1,e):
                            chars = raw_text[nn][-1]
                            if chars == '-':
                                raw_text[nn] = raw_text[nn][:-1] + '{}'.format(label)
                            else:
                                raw_text[nn] = raw_text[nn] + '|{}'.format(label)
                        ...
                        if startchars == '-':
                            ...
                            raw_text[s] = raw_text[s]+'|({}'.format(label)
                        if endchare == '-':
                            raw_text[e] = raw_text[e][:-1]+'{})'.format(label)
                        else:
                            raw_text[e] = raw_text[e]+'|{})'.format(label)
                label += 1
            raw_text = ['{}\n'.format(x) for x in raw_text]
            doc = Document(raw_text, tokens, utts_corefs, utts_speakers, genre, filename)
            ...
            n = 0
            tokens2, utts_corefs2, utts_speakers2 = [], [], []
            st = 0
            en = 0
            shift_token = 0
            shift_token_loc = 0
            for i1, i2 in pairwise([0] + sent_idx):
                if n!= 0 and n % 50 == 0:
                    utts_speakers.extend([speaker] * len(tokens2))

                    raw_text = ['Doc{}_{}\t{}\t-'.format(doc_id, n, ni,
                                                    ) for ii, token in enumerate(tokens2)]
                    ...
                    for groups_index in ruCor.gs[i]['chains']:
                        ...
                        for j in range(len(groups)):
                            ...
                            s = ruCor.words_index[i][dd[0]]
                            e = ruCor.words_index[i][dd[-1]]
                            if s in index and e in index:
                                s -= shift_token_loc
                                e -= shift_token_loc
                                utts_corefs2.append({'label': label, 'start': s, 'end': e, 'span': (s, e)})
                    ...
                    doc = Document(raw_text, tokens2, utts_corefs2, utts_speakers2, genre, filename)
                    documents.append(doc)
                    ni += 1
                    tokens2, utts_corefs2, utts_speakers2 = [], [], []
                    st = en
                    shift_token_loc = shift_token
                tokens2.extend(tokens[i1:i2])
                shift_token += len(tokens[i1:i2])
                n += 1
                en = i2
...
def sents(tokens):
    ...
    sent_idx = sent_idx+[len(tokens)+1]

    # Regroup (returns list of lists)
    return [tokens[i1:i2] for i1, i2 in pairwise([0] + sent_idx)]

def load_dataset(t=True, v=True, te=True, path_dir=''):
    ...
    rucoref_val = rueval.RuCorefCorpus(multeast, rueval)
    ...
    train_corpus, val_corpus, test_corpus = None, None, None
    if v:
        exp_utils.load_corpus(rucoref_val, '../../a_dataset_rucoref/Tokens.txt', '../../a_dataset_rucoref/Groups.txt', numdoc=nDoc[:5])
        val_corpus = rucorToCorpus(rucoref_val)

    if te:
        exp_utils.load_corpus(rucoref_test, '../../a_dataset_rucoref/Tokens.txt', '../../a_dataset_rucoref/Groups.txt', numdoc=nDoc[:5])
        test_corpus = rucorToCorpus(rucoref_test)

    if t:
        m = FastTextKeyedVectors.load("../emb_fasttext/model.model")
        exp_utils.load_corpus(rucoref_train, '../../a_dataset_rucoref/Tokens.txt', '../../a_dataset_rucoref/Groups.txt', numdoc=nDoc[5:])
        train_corpus = rucorToCorpus(rucoref_train)
        with open(os.path.join(path_dir,'train_vocab.txt'), 'w') as f:
            with open('../fasttext_embeddings.ru', 'w') as fw:
                for item in train_corpus.vocab:
                    f.write("%s\n" % item)
                    fw.write("{} {}\n".format(item, np.array_str(m.wv[item], max_line_width = 30000, precision=13))
                                              .replace('[','')
                                              .replace('     ',' ')
                                              .replace('    ',' ')
                                              .replace('   ',' ')
                                              .replace('  ',' ')
                                              .replace(']',''))
        with open(os.path.join(path_dir,'train_char_vocab.txt'), 'w') as f:
            for item in train_corpus.char_vocab:
                f.write("%s\n" % item)

    return train_corpus, val_corpus, test_corpus

def creat_dataset(text):
    return Corpus([creatDoc(text)])

def creatDoc(dtext):

    genre = 'rus'
    speaker = 'speaker'
    utts_corefs, filename = [], ''

    tokens = word_tokenize(dtext)
    utts_speakers = ([speaker] * len(tokens))
    raw_text = ['Doc{}\t{}\t-'.format(1, ii) for ii, token in enumerate(tokens)]

    doc = Document(raw_text, tokens, utts_corefs, utts_speakers, genre, filename)
    return doc


def load_glove(path_dir):
    with open(os.path.join(path_dir,'train_vocab.txt'), 'r') as f:
        vocab = []
        for line in f.readlines():
            vocab.append(line.replace('\n',''))

    GLOVE = LazyVectors.from_corpus(vocab,
                                    name='/home/alex/PycharmProjects/coref/coreference_resolution/fasttext_embeddings.ru',
                                    # name='../multilingual_embeddings.ru',
                                    cache = os.path.join(path_dir,'cache'))
    # GLOVE = LazyVectors.from_modelfastext(set(vocab),
    #                                 name='../emb_fasttext',
    #                                 cache = '../cache')
    return GLOVE
